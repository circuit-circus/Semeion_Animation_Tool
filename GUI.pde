class GUI {
  ...
  class AnimationGraph {                 
    ...
    AnimationGraph(Animation a) {     
      ...
      for (int i = 0; i < myAnimation.myCurves.size(); i++) {
        myCurves.add(new CurveGraph(this, myAnimation.myCurves.get(i), i));
      }
    ...
    void addControlP5() {
      addButton = cp5.addButton("Add")
        .setPosition(width - padding - 40, height/2)
        .setSize(40, 40)
        ;
    }
    ...
    void display() {              
      ...
      if (myAnimation.myCurves.size() > myCurves.size()) {
        myCurves.add(new CurveGraph(this, myAnimation.myCurves.get(myAnimation.myCurves.size()-1), myAnimation.myCurves.size()-1));
      } else if (myAnimation.myCurves.size() < myCurves.size()) {
        myCurves.remove(myCurves.size()-1);
      }
    }

    void removeCurve(int i) {          //<>//
      myAnimation.removeCurve(i);
      myCurves.remove(i);          //<>//
    } 
    ...
    void controlEvent(ControlEvent theEvent) {   
      if (theEvent.isFrom(addButton)) {     
        myAnimation.addCurve();
      }      
      if (!theEvent.isFrom("cp5")) {
        for (CurveGraph c : myCurves) {  
  ...
  class CurveGraph {   
    ...
    CurveGraph(AnimationGraph a, Curve c, int id_) {   
      id = id_;
    ...
    void controlEvent(ControlEvent theEvent) {
      ...
      if (theEvent.isFrom(delButton)) {
        cp.hide();
        superGraph.removeCurve(id);
      }
    }
  }
